...
uniform float lightLeft;
uniform float lightRight;
...
void main() {
  ...
  vec3 color = vec3(0.107*ambientLight, 0.17*ambientLight, 0.27*ambientLight);
  ...
  color = mix(color, vec3(0, 1.0, 1.0)  * lightRight, 0.5 * specular);
  ...
  color = mix(color, vec3(.9, 0.2, 0) * lightLeft, 0.7 * specular);
  ...
  fogFactor = smoothstep( 600.0, 1500.0, depth );
